...
|-------|------|---------|-------------|
| `preset` | `string` | — | Preset name to select. |
| `options.quiet` | `boolean` | `false` | Suppress toast messages. |
| `options.isAuto` | `boolean` | `false` | Mark this as an auto-selection (changes toast wording). |
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `preset` | `string` | — | Context template preset name. |
| `options.quiet` | `boolean` | `false` | Suppress toast messages. |
| `options.isAuto` | `boolean` | `false` | Mark this as an auto-selection. |
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `modelId` | `string` | — | Model name reported by the API. |

**Returns:** `boolean` — `true` if an instruct preset was activated for the model id, `false` otherwise.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `string` | — | Character name for the message. |
| `mes` | `string` | — | Message text. |
| `isUser` | `boolean` | — | `true` if the message is from the user. |
| `isNarrator` | `boolean` | — | `true` if the message is narration/system-rendered. |
| `forceAvatar` | `string` | — | Force-avatar key, used to detect impersonated turns. |
| `name1` | `string` | — | User display name. |
| `name2` | `string` | — | Character display name. |
| `forceOutputSequence` | `boolean \| number` | — | Force first/last output sequence (see `force_output_sequence`). |
| `customInstruct` | `InstructSettings` | `null` | Override instruct settings for this call. |

**Returns:** `string` — message wrapped with instruct-mode sequences.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `storyString` | `string` | — | Story string with anchors already assembled. |
| `params.customContext` | `ContextSettings` | `null` | Override context settings. |
| `params.customInstruct` | `InstructSettings` | `null` | Override instruct settings. |

**Returns:** `string` — story string wrapped with instruct prefix/suffix.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `mesExamplesArray` | `string[]` | — | Array of example message blocks (each potentially `<START>`-delimited). |
| `name1` | `string` | — | User display name. |
| `name2` | `string` | — | Character display name. |

**Returns:** `string[]` — formatted example strings.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `string` | — | Character name to speak as. |
| `isImpersonate` | `boolean` | — | `true` if generating in impersonation mode. |
| `promptBias` | `string` | — | Prompt bias to append. |
| `name1` | `string` | — | User display name. |
| `name2` | `string` | — | Character display name. |
| `isQuiet` | `boolean` | — | `true` if this is a quiet (background) generation. |
| `isQuietToLoud` | `boolean` | — | `true` if this is a quiet-to-loud generation. |
| `customInstruct` | `InstructSettings` | `null` | Override instruct settings. |

**Returns:** `string` — trailing prompt line for the next turn.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `systemPrompt` | `string` | — | The system prompt string. |
| `_customInstruct` | `InstructSettings` | `null` | Ignored (kept for API compatibility). |

**Returns:** `string` — `systemPrompt` or `''` when falsy.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `options.customInstruct` | `InstructSettings` | `null` | Override instruct settings. |
| `options.useStopStrings` | `boolean \| null` | `null` | Whether to include "Chat Start" and "Example Separator"; defaults to `power_user.context.use_stop_strings`. |

**Returns:** `string[]` — stopping sequences to send with the completion request.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `env` | `Object<string, *>` | — | Macro environment map; macro values may also read other env keys. |

**Returns:** `Macro[]` — array of `{{instruct*}}` macro definitions.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `data` | `{ instruct: InstructSettings[] }` | — | Settings payload containing the instruct presets. |

**Returns:** `Promise<void>` — resolves when presets and UI are loaded.
