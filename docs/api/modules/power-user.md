...
`power_user: object` — the global user-settings store. Mutate fields and call `saveSettingsDebounced()` from `../script.js` to persist.
...
| `persona_description_position` | `number` (enum) | Where the persona description is injected (see `persona_description_positions`). |
...
| `personas` | `Record<string, string>` | `{ avatar: name }` — list of saved personas. |
...
| `chat_display` | `number` (enum) | `chat_styles.DEFAULT` / `BUBBLES` / `DOCUMENT`. |
...
| `auto_continue` | `{enabled: boolean, allow_chat_completions: boolean, target_length: number}` | Auto-continue configuration. |
...
| `movingUIState` | `Record<string, object>` | Saved positions/sizes keyed by element id. |
...
| `tokenizer` | `number` (enum) | Active tokenizer (see `../tokenizers.js`). |
| `instruct` | `InstructSettings` | Instruct-template settings. |
| `context` | `ContextSettings` | Context-template settings (story_string, separators, etc.). |
| `sysprompt` | `{enabled: boolean, name: string, content: string, post_history: string}` | System prompt configuration. |
...
| `tag_import_setting` | `number` (enum) | Behavior when importing cards with tags. |
...
`context_presets: ContextPreset[]` — the array of loaded context-template preset objects (`Roleplay`, `Story`, etc.) — each has `name`, `story_string`, `chat_start`, `example_separator`, and the related position/role/depth fields. Populated at boot.
...
| `fuzzySearchCaches` | `Record<string, {resultMap: Map<string, any>}>` | `null` | Optional cache to memoize results. |

**Returns:** `FuseResult<Character>[]` — matched characters with Fuse.js scores.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `data` | `any[]` | — | WI items data array. |
| `searchValue` | `string` | — | The search term. |
| `fuzzySearchCaches` | `Record<string, {resultMap: Map<string, any>}>` | `null` | Optional cache. |

**Returns:** `FuseResult<any>[]` — matched WI entries with Fuse.js scores.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `data` | `string[]` | — | Persona avatar identifiers. |
| `searchValue` | `string` | — | The search term. |
| `fuzzySearchCaches` | `Record<string, {resultMap: Map<string, any>}>` | `null` | Optional cache. |

**Returns:** `FuseResult<any>[]` — matched personas with Fuse.js scores.
...
**Returns:** `FuseResult<Tag>[]` — matched tags with Fuse.js scores.
...
**Returns:** `FuseResult<Group>[]` — matched groups with Fuse.js scores.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `type` | `string` | — | Category key from `fuzzySearchCategories`. |
| `data` | `any[]` | — | Items to search. |
| `keys` | `Array<{name: string, weight: number, getFn?: function}>` | — | Fuse.js keys configuration. |
| `searchValue` | `string` | — | The search term. |
| `fuzzySearchCaches` | `Record<string, {resultMap: Map<string, any>}>` | `null` | Optional cache. |

**Returns:** `FuseResult<any>[]` — search results with scores.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `entities` | `any[]` | — | Array of entities each having an `item` property; sorted in place. |
| `forceSearch` | `boolean` | — | Force using search-score order. |
| `filterHelper` | `FilterHelper` | `null` | Filter helper providing search scores; defaults to `entitiesFilter`. |
...
Compiles the active context preset's `story_string` (Handlebars) with `params` (typically `{ description, personality, scenario, persona, system, wiBefore, wiAfter, char, user }`), runs `substituteParams` for any unresolved `{{macros}}`, and normalizes trailing newlines. Throws on Handlebars compile errors (and toasts the user).

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `params` | `object` | — | Template parameters (`description`, `personality`, `scenario`, `persona`, `system`, `wiBefore`, `wiAfter`, `char`, `user`, ...). |
| `options.customStoryString` | `string` | `null` | Override the active preset's `story_string`. |
| `options.customInstructSettings` | `InstructSettings` | `null` | Override instruct settings for this render. |
| `options.customContextSettings` | `ContextSettings` | `null` | Override context settings for this render. |

**Returns:** `string` — the rendered story string with trailing newline normalized.
...
**Returns:** `object` — flattened snapshot of all context-control values.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `value` | `string` | — | The stopping string to queue. |

**Returns:** `void` (no-op if `value` is already queued).
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `limit` | `number` | `undefined` | Max number of strings to return; `0`/`undefined` returns all. |

**Returns:** `string[]` — combined permanent + ephemeral stopping strings.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `text` | `string` | — | Generated text to check. |

**Returns:** `boolean` — `true` if the text should be auto-rejected.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `text` | `string` | — | Text to repair. |
| `forDisplay` | `boolean` | — | When `true`, also closes unpaired `*` and `"` on each line. |

**Returns:** `string` — the repaired text.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `x` | `string` | — | String to process. |

**Returns:** `string` — input with `\n+` collapsed to `\n`.
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `id` | `string` | — | Element id whose inline styles should be reset. |

**Returns:** `void` (no-op if the element is missing).
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `functionId` | `string` | — | Unique id for the entry. |
| `name` | `string` | — | Display label in the Debug menu. |
| `description` | `string` | — | Help text shown alongside the entry. |
| `func` | `function` | — | Callback invoked when the entry is clicked. |
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `param.force` | `boolean` | `undefined` | Bypass the `play_message_sound` and focus checks. |
